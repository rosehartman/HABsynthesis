...
microb = left_join(microcystis, DF, by = c("Collection_Date" = "Date")) %>%
  mutate(CVPSWP = CVP + SWP)

#plot microcystis versus X2
ggplot(microb, aes(x = X2, y = log(dwr.MIC_totbvL+1), 
                  color = as.factor(Survey_Year)))+
  geom_point()
...
micro2a = mutate(microb, logBV = log(dwr.MIC_totbvL/100000+1), Year = as.factor(Survey_Year), DOY = yday(Collection_Date)) %>%
...
#rescale variables
microa = mutate(micro2a, Temp = scale(field.Water.temp), X2s = scale(X2), Outs = scale(log(OUT)),
...
microa = mutate(microa, SACs = scale(SAC), SJRs = scale(SJR)) %>%
  left_join(DFspring) %>%
...
microa = mutate(microa, Month = month(Collection_Date)) %>%
  filter(Month !=6) 

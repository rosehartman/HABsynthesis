...
microtest = as.data.frame(micro[,c(2:6,8:10, 14)])
...
Years = data.frame(Year = as.factor(2014:2019))
...
test2$predictions = predict(bestmod2, newdata = test2)
...
ggplot(filter(newdata, Year == 2016, CVPSWP ==1650), aes(x = SJR, y = BV, color = as.factor(TempNotScaled))) + geom_line()+
  facet_wrap(~Month)
...
px = ggplot(filter(newdataPlot, Month == 8, CVPSWP ==6250), aes(x = SJR/1000, y = BV/1000, color = as.factor(TempNotScaled))) + 
  geom_line()+
  facet_wrap(~Year)+
...
p2 = visreg(bestmod2, gg = T)

ptemp = p2[[1]]$data
pexp = p2[[2]]$data
psjr = p2[[3]]$data
pyr = p2[[4]]$plot_env 
test = ggplot_build(p2[[4]])$data


ggplot(test[[7]], aes(x = x, y = y))+ geom_point()


preds = bind_cols(micro, ptemp)
preds2 = bind_cols(micro, psjr)
preds3 = bind_cols(micro, exp)

preds4 = bind_cols(micro, pyr)

r.squaredGLMM(bestmod2)
...
yeardat = bind_cols(micro, test[[7]]) %>%
  mutate(parameter = "Year")
p2[[4]]+ theme_bw()+
  geom_point(data = yeardat, aes(x = x, y = y, color = Year, shape = Year), inherit.aes = FALSE)+
  scale_color_viridis_d(option = "B", guide = NULL)+
  scale_shape(guide = NULL)+
  ylab(ylabM)+
  facet_wrap(~parameter)
ggsave("plots/PartialResid_year.tiff", device = "tiff", width =4, height =5)
